...
class Softmax_xz(nn.Module):
    def __init__(self, in_features, out_features, device):
        super(Softmax_xz, self).__init__()
        self.weight_fc = Parameter(torch.FloatTensor(out_features, in_features),requires_grad=True)
        print('fc_shape', self.weight_fc.shape)
        print('fc_grad', self.weight_fc.requires_grad)
        nn.init.xavier_uniform_(self.weight_fc)

    def forward(self, x, labels):
        weight_fc_norm = F.normalize(self.weight_fc, dim=1)
        cosine = F.linear(x, weight_fc_norm)
        return cosine
...
class Softmax(nn.Module):
    ...
    def forward(self, x, labels):
        ...
        out = F.linear(F.normalize(x), F.normalize(self.weight), self.bias)
        return out
