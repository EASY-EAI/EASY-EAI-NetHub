...
class classification_script(object):
    ...
    def __init__(self, model, loss_function, train_loader, eval_loader,
                 optimizer, lr_scheduler, device, save_path):
        ...
        self.activation_step = {'print': 50, 'save': 400, 'eval': 400}
    ...
    def train(self, epoch):
        ...
        print(' **start train')
        print(' **train sample: {}'.format(len(self.train_loader.dataset)))
        ...
        for i, (inputs, target) in enumerate(self.train_loader):
            ...
            prec1, prec5 = accuracy(output.data, target_var, topk=(1, self.topk_number))
            ...
            if i % self.activation_step['print'] == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                      ...
                      'Prec@{topk_number} {top5.val:.3f} ({top5.avg:.3f})'.format(self.epoch, i, len(self.train_loader), len(self.train_loader),
                                                                                  batch_time=self.batch_time, data_time=self.data_time,
                                                                                  loss=self.losses, top1=self.top1, topk_number=self.topk_number,top5=self.top5))


            if i % self.activation_step['save'] == 0 and i!=0:
                self.save_model(os.path.join(self.save_path, 'experiment0_last.pth'))
            # log
            if i % self.activation_step['eval'] == 0 and i!=0:
                ...
                if top1 > self.best_result:
                    self.save_model(os.path.join(self.save_path, 'experiment0_best.pth'))
                self.set_train()
    ...
    def eval(self):
        ...
        print(' **start eval')
        print(' **eval sample: {}'.format(len(self.eval_loader.dataset)))
        ...
        print('Eval result on {} datas:\t'
              ...
              'Loss {loss.avg:.4f}\t'
              'Prec@1 {top1.val:.3f} ({top1.avg:.3f})\t'
              'Prec@5 {top5.val:.3f} ({top5.avg:.3f})\n'.format(len(self.eval_loader.dataset),
        ...
        if _top1.avg > self.eval_best:
            self.eval_best = _top1.avg
            self.save_model(os.path.join(self.save_path, 'eval_best.pth'))
        return _top1.avg

    def run_on_single_img(self, img_cv):
    ...
    def export_onnx(self, path):
        virtual_input = Variable(torch.randn(1, 3, 112, 112), requires_grad=True).to(self.device)
        torch.onnx.export(self.model, virtual_input, path)

    def export_pt(self, path):
        # save visilization_model
        example_forward_input = torch.rand(1, 3, 112, 112).to(self.device)
        traced_script_module = torch.jit.trace(self.model, example_forward_input)
